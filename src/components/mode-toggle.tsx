...
export type ModeToggleTheme = 'light' | 'dark'

export interface ModeToggleAppBarProps {
...
export interface ModeToggleProps {
  ...
  appBarProps?: ModeToggleAppBarProps
  /** Starting theme. Default: "dark" */
  defaultTheme?: ModeToggleTheme
  ...
  onThemeChange?: (theme: ModeToggleTheme) => void
...
const TOKENS: Record<ModeToggleTheme, Record<string, string>> = {
  light: {
    pageBg: '#f3ede1',
    pageText: '#1a1a1a',
    barBg: '#1a1a1a',
...
type TogglePhase = 'idle' | 'falling' | 'rising'

const EASING = 'cubic-bezier(0.76, 0, 0.24, 1)'

function resolveModeToggleTheme(value: string | undefined): ModeToggleTheme {
...
export function ModeToggle({
  ...
  const [theme, setTheme] = useState<ModeToggleTheme>(defaultTheme)
  ...
  useEffect(() => {
    if (!mounted) return
    setTheme(resolveModeToggleTheme(resolvedTheme))
  }, [mounted, resolvedTheme])

  const toggle = useCallback(() => {
    if (phase !== 'idle') return
    const next: ModeToggleTheme = theme === 'light' ? 'dark' : 'light'
